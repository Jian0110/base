...
function(a, b) {
	...
	ace.demo.functions.basics = function() {
		ace.vars.non_auto_fixed && a("body").addClass("mob-safari"),
		ace.vars.transition = ace.vars.animation || !!a.support.transition
	},
	ace.demo.functions.enableSidebar = function() {
		var b = a(".sidebar");
		a.fn.ace_sidebar && b.ace_sidebar(),
		a.fn.ace_sidebar_scroll && b.ace_sidebar_scroll({
	...
	ace.demo.functions.handleScrollbars = function() {
		...
		b && !ace.vars.old_ie && (a(window).on("resize.reset_scroll",
		function() {
			a(".ace-scroll:not(.scroll-disabled)").not(":hidden").ace_scroll("reset")
		}), b && a(document).on("settings.ace.reset_scroll",
		function(b, c) {
	...
	ace.demo.functions.dropdownAutoPos = function() {
		a(document).on("click.dropdown.pos", '.dropdown-toggle[data-position="auto"]',
		function() {
			var b = a(this).offset(),
			c = a(this.parentNode);
	...
	ace.demo.functions.navbarHelpers = function() {
		a('.ace-nav [class*="icon-animated-"]').closest("a").one("click",
		function() {
			var b = a(this).find('[class*="icon-animated-"]').eq(0),
			c = b.attr("class").match(/icon\-animated\-([\d\w]+)/);
			b.removeClass(c[0])
		}),
		a(document).on("click", ".dropdown-navbar .nav-tabs",
		function(b) {
			b.stopPropagation();
			var c;
			b.target; (c = a(b.target).closest("[data-toggle=tab]")) && c.length > 0 && (c.tab("show"), b.preventDefault(), a(window).triggerHandler("resize.navbar.dropdown"))
		})
	},
	...
	ace.demo.functions.someBrowserFix = function() {
		if (ace.vars.webkit) {
			...
			b && a(window).on("resize.webkit_fix",
			function() {
				ace.helper.redraw(b)
			})
		}
		ace.vars.ios_safari && a(document).on("ace.settings.ios_fix",
		function(b, c, d) {
			"navbar_fixed" == c && (a(document).off("focus.ios_fix blur.ios_fix", "input,textarea,.wysiwyg-editor"), 1 == d && a(document).on("focus.ios_fix", "input,textarea,.wysiwyg-editor",
			function() {
				a(window).on("scroll.ios_fix",
				function() {
	...
	ace.demo.functions.bsCollapseToggle = function() {
		...
		function(b) {
			...
			0 != d.length && d.find(ace.vars[".icon"]).each(function() {
				var c, d = a(this),
				e = null,
				f = null;
				return (e = d.attr("data-icon-show")) ? f = d.attr("data-icon-hide") : (c = d.attr("class").match(/fa\-(.*)\-(up|down)/)) && (e = "fa-" + c[1] + "-down", f = "fa-" + c[1] + "-up"),
	...
	ace.demo.functions.smallDeviceDropdowns = function() {
		function b() {
			...
			if ("fixed" == b.css("position")) {
				...
				if (1 == k.length) {
					...
					n && (m = n.scrollHeight),
					j += l.find(".dropdown-header").outerHeight(),
					j += l.find(".dropdown-footer").outerHeight();
		...
		ace.vars.old_ie || a(document).on("shown.bs.dropdown.navbar", ".ace-nav > li.dropdown-modal",
		function(c) {
			b.call(this);
			var d = this;
			a(window).on("resize.navbar.dropdown",
			function() {
				b.call(d)
			})
		}).on("hidden.bs.dropdown.navbar", ".ace-nav > li.dropdown-modal",
		function(b) {
			a(window).off("resize.navbar.dropdown"),
			c.call(this)
		})
	}
} (jQuery),
...
function(a, b) {
	function c(b, c) {
		...
		function f(b) {
			...
			if (c.length > 0) document.title = c.text(),
			c.remove();
			else if (b.length > 0) {
		...
		this.reload = function() {
			var b = a.trim(window.location.hash); ! b && this.settings.default_url && (b = this.settings.default_url),
			this.loadUrl(b, !1)
		},
		...
		this.getUrl = function(b, c, d, i, l, m, n) {
			if (!j) {
				...
				if (g.trigger(o = a.Event("ajaxloadstart"), {
					...
					"POST" == i && "object" == typeof n && (p = a.extend({},
					p, n)),
					a.ajax(p).error(function() {
						...
						if ("function" == typeof h.settings.update_active) o = h.settings.update_active.call(null, c, b, i, m);
							...
							if (q.length > 0) {
								q.find(".active").each(function() {
									var b = "active"; (a(this).hasClass("hover") || h.settings.close_active) && (b += " open"),
						...
						m && (j = String(j).replace(/<(title|link)([\s\>])/gi, '<div class="hidden ajax-append-$1"$2').replace(/<\/(title|link)\>/gi, "</div>"), g.empty().html(j)),
		...
		this.stopLoading = function(a) {
			a === !0 ? (j = !1, k.remove(), l && (g.css("position", ""), l = !1), null != m && (clearTimeout(m), m = null)) : (k.addClass("almost-loaded"), g.one("ajaxscriptsloaded.inner_call",
			function() {
				h.stopLoading(!0)
			}))
		},
		...
		a(window).off("hashchange.ace_ajax").on("hashchange.ace_ajax",
		function(b, c) {
		...
		var n = a.trim(window.location.hash); ! n && this.settings.default_url && (window.location.hash = this.settings.default_url)
	}
	var d = {};
...
function(a, b) {
	function c(b, c) {
		...
		o.call(f),
		a(window).on("resize.sidebar.vars",
		function() {
			e.call(f)
		}).triggerHandler("resize.sidebar.vars"),
		this.$sidebar.on(ace.click_event + ".ace.submenu", ".nav-list",
		function(b) {
			var c = this,
			...
			if (d && 0 != d.length) {
				...
				if ("tap" == ace.click_event && e && d.get(0).parentNode.parentNode == c) {
					...
					if (null != l && b.target != l && !a.contains(l, b.target)) return b.preventDefault(),
					!1
				}
				if (ace.vars.ios_safari && "false" !== d.attr("data-link")) return document.location = d.attr("href"),
				b.preventDefault(),
				!1
			}
		})
	...
	var d = 0;
	a(document).on(ace.click_event + ".ace.menu", ".menu-toggler",
	function(b) {
		var c = a(this),
		...
		if (0 != d.length) {
			...
			return c.hasClass("display") ? (f && a(document).on(e,
			function(b) {
	...
	function(b) {
		...
		if (c && (d = a(c)), null != d && 0 != d.length || (d = a(this).closest(".sidebar")), 0 != d.length) {
			...
			"true" === d.attr("data-auto-hide") && (d.hasClass(responsive_max_class) ? a(document).on(f,
			function(b) {
...
function(a, b) {
	function c(b, c) {
		...
		if (f = l.find(".nav-list"), g = f.get(0)) {
			...
			this.settings = a.extend({},
			a.fn.ace_sidebar_scroll.defaults, c, m);
			...
			var w, x, y = function() {
				...
				return j.sidebar_fixed && (a.top -= ace.helper.scrollTop()),
				k.innerHeight() - a.top - (j.settings.include_toggle ? 0 : h.outerHeight()) + 1
			},
			z = function() {
			...
			A = function(b) {
				if (!v && j.sidebar_fixed && (f = l.find(".nav-list"), h = l.find(".sidebar-toggle").eq(0), i = l.find(".sidebar-shortcuts").eq(0), g = f.get(0))) {
					...
					j.settings.include_toggle || h.css({
						"z-index": 1
					}),
					...
					f.on(a.event.special.mousewheel ? "mousewheel.ace_scroll": "mousewheel.ace_scroll DOMMouseScroll.ace_scroll",
					function(a) {
						return j.is_scrolling && u.is_active() ? p.trigger(a) : !j.settings.lock_anyway
					}),
					f.on("mouseenter.ace_scroll",
					function() {
						t.addClass("scroll-hover")
					}).on("mouseleave.ace_scroll",
					function() {
						t.removeClass("scroll-hover")
					});
					var c = q.get(0);
					if (f.on("ace_drag.nav",
					function(b) {
						...
						if ("up" == b.direction || "down" == b.direction) {
							u.move_bar(!0);
							var d = b.dy;
							d = parseInt(Math.min(w, d)),
							Math.abs(d) > 2 && (d = 2 * d),
							0 != d && (c.scrollTop = c.scrollTop + d, g.style.top = -1 * c.scrollTop + "px")
						}
					}), j.settings.smooth_scroll && f.on("touchstart.nav MSPointerDown.nav pointerdown.nav",
					function(a) {
			...
			this.scroll_to_active = function() {
				if (u && u.is_active()) try {
					...
					var d = a.outerHeight();
					c = c.get(0);
					for (var e = a.get(0); e != c;) d += e.offsetTop,
					e = e.parentNode;
					var h = d - p.height();
			...
			this.disable = function() {
				...
				parseInt(g.style.top) < 0 && j.settings.smooth_scroll && a.support.transition.end ? f.one(a.support.transition.end,
				function() {
			...
			this.prehide = function(a) {
				if (this.is_scrolling && !o.get("minimized")) if (z() + a < y()) this.disable();
				else if (0 > a) {
					var b = q.scrollTop() + a;
					if (0 > b) return;
					g.style.top = -1 * b + "px"
			...
			l.on("hidden.ace.submenu.sidebar_scroll shown.ace.submenu.sidebar_scroll", ".submenu",
			function(a) {
				a.stopPropagation(),
				o.get("minimized") || (j._reset(), "shown" == a.type && j.set_hover())
			}),
			A(!0)
		}
	}
	var d = ace.vars.safari && navigator.userAgent.match(/version\/[1-5]/i),
	e = "getComputedStyle" in window ?
	function(a, b) {
	...
	a(document).on("settings.ace.sidebar_scroll",
	function(b, c, d) {
		a(".sidebar[data-sidebar-scroll=true]").each(function() {
			var b = a(this),
			d = b.ace_sidebar_scroll("ref");
			if ("sidebar_collapsed" == c && e(this, "fixed"))"true" == b.attr("data-sidebar-hover") && b.ace_sidebar_hover("reset"),
			d._reset();
			else if ("sidebar_fixed" === c || "navbar_fixed" === c) {
				var f = d.get("is_scrolling"),
				g = e(this, "fixed");
				d.set("sidebar_fixed", g),
				g && !f ? d._reset() : g || d.disable()
			}
		})
	}),
	a(window).on("resize.ace.sidebar_scroll",
	function() {
		a(".sidebar[data-sidebar-scroll=true]").each(function() {
			var b = a(this);
	...
	a.fn.ace_sidebar_scroll || (a.fn.ace_sidebar_scroll = function(d, e) {
		var f, g = this.each(function() {
			...
			h = "object" == typeof d && d;
			g || b.data("ace_sidebar_scroll", g = new c(this, h)),
...
function(a, b) {
	function c(b, c) {
		...
		function k(c) {
			...
			!C && (v = t - (y + h)) >= 0 && (v = z > v ? v: z, 0 == v && (v = 20), z - v > 10 && (v += parseInt(Math.min(25, z - v))), m.top + (n - o) > t - v && (v -= m.top + (n - o) - (t - v)), v > 0 && (c.style.top = -v + "px", g && (g.style.marginTop = -v + "px"))),
			0 > v && (v = 0);
			var D = v > 0 && v > n - 20;
			if (D ? d.addClass("pull_up") : d.removeClass("pull_up"), B) if (d.parent().parent().hasClass("hover-flip")) d.addClass("hover-flip");
			else {
				var E = f.offset(),
				F = f.width(),
				G = u.width();
				...
				if (J > 0 && n > J && (I += parseInt(Math.max(n, n - J))), I -= 5, !(90 > I)) {
					...
					if (x = I, !e && K) {
						...
						if (L) {
							...
							C && (L.style.left = parseInt(M - 2) + "px", L.style.top = parseInt(m.top) + (g ? o - 2 : 0) + "px")
						}
					}
		...
		this.settings = a.extend({},
		a.fn.ace_sidebar_hover.defaults, c, n);
		var o = a(b),
		...
		var x = -1;
		e || o.on("hide.ace.submenu.sidebar_hover", ".submenu",
		function(b) {
			if (! (1 > x)) {
				b.stopPropagation();
		...
		e || o.on("shown.ace.submenu.sidebar_hover hidden.ace.submenu.sidebar_hover", ".submenu",
		function(b) {
			if (! (1 > x)) {
				var c = a(this).closest(".ace-scroll.can-scroll");
		...
		o.on(z, ".nav-list li, .sidebar-shortcuts",
		function(b) {
		...
		}).on(A, ".nav-list li, .sidebar-shortcuts",
		function(b) {
...
function(a, b) {
	function c(b, c) {
		...
		if (c) {
			...
			g = parseInt(g),
			d.css("min-height", g),
			e ? (f && d.ace_scroll("disable"), d.css("max-height", g).addClass("overflow-scroll")) : (f ? d.ace_scroll("update", {
				size: g,
				mouseWheelLock: !0,
				lockAnyway: !0
			}) : d.ace_scroll({
				...
				mouseWheelLock: !0,
				lockAnyway: !0
			}), d.ace_scroll("enable").ace_scroll("reset")),
			a(window).on("resize.widget.scroll",
			function() {
				var a = b.height() - b.find(".widget-header").height() - 10;
				a = parseInt(a),
				d.css("min-height", a),
				e ? d.css("max-height", a).addClass("overflow-scroll") : d.ace_scroll("update", {
					size: a
				}).ace_scroll("reset")
			})
		} else {
	...
	var d = function(b, d) {
		...
		this.toggle = function(a, b, c, d) {
			...
			if (b || (b = e.find("> .widget-header a[data-action=collapse]").eq(0), 0 == b.length && (b = null)), b) {
				...
				var j, k = null,
				l = null; (k = g.attr("data-icon-show")) ? l = g.attr("data-icon-hide") : (j = g.attr("class").match(/fa\-(.*)\-(up|down)/)) && (k = "fa-" + j[1] + "-down", l = "fa-" + j[1] + "-up")
			}
			var c = "undefined" == typeof c ? 250 : c,
			d = "undefined" == typeof d ? 200 : d;
			"show" == h ? (g && g.removeClass(k).addClass(l), f.hide(), e.removeClass("collapsed"), f.slideDown(c,
			function() {
				e.trigger(i + ".ace.widget")
			})) : (g && g.removeClass(l).addClass(k), f.slideUp(d,
			function() {
		...
		this.fullscreen = function(a) {
			...
			d = null,
			e = null; (d = b.attr("data-icon1")) ? e = b.attr("data-icon2") : (d = "fa-expand", e = "fa-compress");
			var f = this.$box.hasClass("fullscreen"),
			g = a !== !0 && a !== !1;
			a === !0 || g && !f ? (b.removeClass(d).addClass(e), this.$box.addClass("fullscreen"), c(this.$box, !0)) : (a === !1 || g && f) && (b.addClass(d).removeClass(e), this.$box.removeClass("fullscreen"), c(this.$box, !1)),
	...
	a.fn.widget_box = function(c, e) {
		var f, g = this.each(function() {
			...
			g = b.data("widget_box"),
			h = "object" == typeof c && c;
			g || b.data("widget_box", g = new d(this, h)),
			"string" == typeof c && (f = g[c](e))
		});
		return f === b ? g: f
	},
	a(document).on(ace.click_event + ".ace.widget", ".widget-header a[data-action]",
	function(b) {
		b.preventDefault();
		var c = a(this),
		...
		if (0 != e.length && !e.hasClass("ui-sortable-helper")) {
			...
			if ("collapse" == g) {
				...
				e.trigger(h = a.Event("toggle.ace.widget"), [i]),
				f.toggle(i, c)
			} else if ("close" == g) {
				var h;
...
function(a, b) {
	...
	a("#ace-settings-compact").on("click",
	function() {
		if (this.checked) {
			a("#sidebar").addClass("compact");
			var b = a("#ace-settings-hover");
			b.length > 0 && b.removeAttr("checked").trigger("click")
		} else a("#sidebar").removeClass("compact"),
		a("#sidebar[data-sidebar-scroll=true]").ace_sidebar_scroll("reset");
		ace.vars.old_ie && ace.helper.redraw(a("#sidebar")[0], !0)
	}),
	a("#ace-settings-highlight").on("click",
	function() {
		this.checked ? a("#sidebar .nav-list > li").addClass("highlight") : a("#sidebar .nav-list > li").removeClass("highlight"),
		ace.vars.old_ie && ace.helper.redraw(a("#sidebar")[0])
	}),
	a("#ace-settings-hover").on("click",
	function() {
		if (!a("#sidebar").hasClass("h-sidebar")) {
			...
			a(".sidebar[data-sidebar-hover=true]").ace_sidebar_hover("reset"),
			a(".sidebar[data-sidebar-scroll=true]").ace_sidebar_scroll("reset"),
			ace.vars.old_ie && ace.helper.redraw(a("#sidebar")[0])
		}
	});
	a(document).on("settings.ace",
	function(b, c, d, e, f) {
		var g = "";
		switch (c) {
		case "navbar_fixed":
	...
	ace.settingFunction = {
		navbar_fixed: function(b, c, d, e) {
			...
			c ? a(b).addClass("navbar-fixed-top") : a(b).removeClass("navbar-fixed-top"),
			d && ace.settings.saveState(b, "class", "navbar-fixed-top", c),
			a(document).trigger("settings.ace", ["navbar_fixed", c, b, d])
		},
		...
		breadcrumbs_fixed: function(b, c, d, e) {
			...
			var f, c = c || !1,
			d = "undefined" != typeof d ? d: !0;
			return a(document).trigger(f = a.Event("presettings.ace"), ["breadcrumbs_fixed", c, b, d]),
			f.isDefaultPrevented() ? !1 : (c && e !== !1 && ace.settingFunction.sidebar_fixed(null, !0, d), c ? a(b).addClass("breadcrumbs-fixed") : a(b).removeClass("breadcrumbs-fixed"), d && ace.settings.saveState(b, "class", "breadcrumbs-fixed", c), void a(document).trigger("settings.ace", ["breadcrumbs_fixed", c, b, d]))
		},
		main_container_fixed: function(b, c, d) {
			...
			navigator.userAgent.match(/webkit/i) && (a("#sidebar").toggleClass("menu-min"), setTimeout(function() {
...
function(a, b) {
	...
	var c = function() {
		function b(b) {
			...
			b = b || document.body;
			var e = a(b);
			e.find(".dropdown-menu:not(.datepicker-dropdown,.colorpicker)").toggleClass("dropdown-menu-right").end().find(".pull-right:not(.dropdown-menu,blockquote,.profile-skills .pull-right)").removeClass("pull-right").addClass("tmp-rtl-pull-right").end().find(".pull-left:not(.dropdown-submenu,.profile-skills .pull-left)").removeClass("pull-left").addClass("pull-right").end().find(".tmp-rtl-pull-right").removeClass("tmp-rtl-pull-right").addClass("pull-left").end().find(".chosen-select").toggleClass("chosen-rtl").next().toggleClass("chosen-rtl"),
			...
			e.find(".fa").each(function() {
				if (! (this.className.match(/ui-icon/) || a(this).closest(".fc-button").length > 0)) for (var b = this.attributes.length,
				c = 0; b > c; c++) {
			...
			f ? (e.find(".scroll-hz").addClass("make-ltr").find(".scroll-content").wrapInner('<div class="make-rtl" />'), a(".sidebar[data-sidebar-hover=true]").ace_sidebar_hover("changeDir", "right")) : (e.find(".scroll-hz").removeClass("make-ltr").find(".make-rtl").children().unwrap(), a(".sidebar[data-sidebar-hover=true]").ace_sidebar_hover("changeDir", "left")),
		...
		b(),
		a(".page-content-area[data-ajax-content=true]").on("ajaxscriptsloaded.rtl",
		function() {
...
function(a, b) {
	...
	function() {
		function b(b) {
			...
			a(".ace-nav > li:not(:last-child)").removeClass("light-pink").find("> a > " + ace.vars[".icon"]).removeClass("pink").end().eq(0).find(".badge").removeClass("badge-warning"),
			...
			} else if ("skin-2" == b) a(".ace-nav > li").addClass("no-border margin-1"),
			a(".ace-nav > li:not(:last-child)").addClass("light-pink").find("> a > " + ace.vars[".icon"]).addClass("pink").end().eq(0).find(".badge").addClass("badge-warning"),
			...
			a(".sidebar[data-sidebar-hover=true]").ace_sidebar_hover("updateStyle", "no-track scroll-thin scroll-white");
				...
				a(".sidebar-shortcuts .btn").each(function() {
					...
					a(this).find(ace.vars[".icon"]).addClass(d[e++])
				}),
...
function(a, b) {
	...
	try {
		a("#nav-search-input").bs_typeahead({
			...
			updater: function(b) {
				return a("#nav-search-input").focus(),
				b
			}
		})
	} catch(c) {}
